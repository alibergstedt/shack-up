...
CREATE PROCEDURE ListingsInsert (
	@ListingId int output,  /* great for redirects - passes out the id */
	@UserId nvarchar(128),
	@StateId char(2),
	@BathroomTypeId int,
...
BEGIN
	...
	SET @ListingId = SCOPE_IDENTITY(); /* returns last id identity that was created */
...
BEGIN
	BEGIN TRANSACTION /* Runs as an all or nothing batch */
...
BEGIN
	SELECT ListingId, UserId, StateId, BathroomTypeId, Nickname, City, Rate,
	SquareFootage, HasElectric, ListingDescription, HasHeat, ImageFileName
	FROM Listings
	WHERE ListingId = @ListingId
END
GO
...
BEGIN
	SELECT ListingId, UserId, Nickname, City, StateId, Rate, SquareFootage, HasElectric, HasHeat,
	l.BathroomTypeId, BathroomTypeName, ImageFileName, l.ListingDescription
	FROM Listings l
...
BEGIN
	SELECT l.ListingId, l.City, l.StateId, l.Rate, l.SquareFootage, l.HasElectric,
	l.UserId, l.HasElectric, l.HasHeat, l.BathroomTypeId, b.BathroomTypeName
	FROM Favorites f
...
BEGIN
	SELECT ListingId, UserId, Nickname, City, StateId, Rate, SquareFootage, HasElectric, HasHeat,
	l.BathroomTypeId, BathroomTypeName, ImageFileName
	FROM Listings l
		INNER JOIN BathroomTypes b ON l.BathroomTypeId = b.BathroomTypeId
	WHERE UserId = @UserId
END
